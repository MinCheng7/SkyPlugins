...
function BahamutSign() { //查询巴哈姆特签到Token
	return $.http.get({ 
		...
		.catch(err => {
            $.hasError = true;
			$.notifyMsg.push(`❌ 主页签到 丨 ${err.message||err}`); 
	...
	return $.http.get({ 
		...
		.then(async (resp) => { 
			...
			if (list.length) { 
				...
				let guildRes = `⭕ 公会签到 丨 成功 ${sucsCount} 个`;
				if (failGuilds.length > 0) {
		...
		.catch(err => { 
			$.hasError = true;
			$.notifyMsg.push(`❌ 公会签到 丨 ${err.message || err}`); 
...
function StartSignGuild(v) { //巴哈姆特公会签到
	return $.http.post({ 
		...
		.then((res) => { 
			...
			// 核心优化：底层函数只干活，不在内部直接调用 $.log 打印日志，而是将状态打包抛给上层统一处理
			if (body.ok) { 
		...
		.catch(e => { 
			return { status: 'fail', name: v.name, msg: e.message || e }; 
		});
}
	...
	return $.http.get({ 
		...
		.catch(e => { 
			...
			if (errorMsg.includes('一天僅限一次機會') || errorMsg.includes('答過')) {
				...
				$.hasError = true;
				$.notifyMsg.push(`❌ 动画答题丨${errorMsg}`); 
				$.log('', `❌ 动画疯答题失败`, `❌${errorMsg}`); 
			}
		});
}
