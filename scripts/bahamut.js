...
// 静音底层默认的通知文案复制打印
$.isMuteLog = true; 
// 覆写结束函数，去除多余的运行秒数和等号分隔线
$.done = (t = {}) => {
    if (typeof $done !== 'undefined') $done(t);
};
...
(async function() { // 立即运行的匿名异步函数
    ...
    $.hasError = true;
    $.notifyMsg.push(`❌ 运行异常丨${e.message || e}`);
...
async function BahamutLogin(retry = 3, interval = 1000) { //登录函数，拿到Set-Cookie
	for (let i = 0; i < retry; i++) { //循环登录(默认三次)
		...
		const res = await $.http.post(reqUrl) 
			...
			}).catch((err) => `❌ 登录失败, ${err.message || err}`);
		$.log('', res.message || res);
		if (res === `✅ 巴哈姆特登录成功`) {
			break; 
		} else if (retry == i + 1) { 
...
function BahamutSign() { //查询巴哈姆特签到Token
	return $.http.get({ 
			...
			if (resp.body) { 
				$.log('', '🛡️ 获取签到令牌成功'); 
				// 接收修改后返回的具体文本
				const signRes = await StartSignBahamut(resp.body); 
				$.notifyMsg.push(`⭕ 主页签到丨${signRes}`); 
		...
		.catch(err => {
            $.hasError = true;
			$.notifyMsg.push(`❌ 主页签到失败, ${err.message||err}`); 
...
function StartSignBahamut(token) { //巴哈姆特签到
	return $.http.post({ 
		...
		.then(res => { 
			...
			if (body.data) { 
				$.log('', '✅ 巴哈姆特签到成功', `丨已连续签到${body.data.days}天`); 
				return `已连续签到 ${body.data.days} 天`; 
			} else { 
				const failMsg = body.error ? body.error.message : null; 
				const errorTxt = failMsg || body.message || '';
				// 核心修改：如果是已签过，不抛出异常，返回特制文案
				if (errorTxt.includes('已經簽到')) {
					$.log('', '✅ 今天已经签到过了');
					return '今天已签过';
				}
				throw new Error(errorTxt || '未知'); 
			}
		}); 
}
...
function StartAdsBonus(token, type) {
	...
	return $.http.post({ 
		...
		.catch(err => {
            $.hasError = true;
			$.notifyMsg.push(`❌ 广告签到失败, ${err.message||err}`); 
...
function BahamutGuildSign() { //巴哈姆特查询公会列表
	...
	return $.http.get({ 
			...
			const list = (resp.body.replace(/\n/g, '').match(/guild\.php\?g?sn=\d.+?<\/p>/g) || []) 
				.map(n => { 
					return { 
						...
						// 视觉优化：顺手净化网页的转义字符，让公会名字更清爽
						name: n.split(/<p>(.+?)<\/p>/)[1].replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
					}
				});
			if (list.length) { 
				$.log('', `⭕ 获取公会列表成功`); 
				...
				// 对底层返回的结果进行分类
				const successGuilds = signResults.filter(r => r.status === 'success');
				const alreadyGuilds = signResults.filter(r => r.status === 'already');
				const failGuilds = signResults.filter(r => r.status === 'fail');

				// 核心优化：日志合并与分类打印
				if (successGuilds.length > 0) {
					successGuilds.forEach(r => $.log('', `🏛️ ${r.name}`, `✅ ${r.msg}`));
				}
				if (alreadyGuilds.length > 0) {
					const names = alreadyGuilds.map(r => r.name).join(', ');
					$.log('', `🏛️ 已经签到过的公会: [${names}]`);
				}
				if (failGuilds.length > 0) {
					failGuilds.forEach(r => $.log('', `🏛️ ${r.name}`, `❌ 签到失败: ${r.msg}`));
					$.hasError = true;
				}
				
				// 拼接弹窗通知
				const sucsCount = successGuilds.length + alreadyGuilds.length; 
				let guildRes = `⭕ 公会签到丨成功 ${sucsCount} 个`;
				if (failGuilds.length > 0) {
					const failNames = failGuilds.map(r => r.name).join('、');
					guildRes += `，失败 ${failGuilds.length} 个 (${failNames})`;
				}
		...
		.catch(err => { 
			$.hasError = true;
			$.notifyMsg.push(`❌ 公会签到失败, ${err.message || err}`); 
...
function StartSignGuild(v) { //巴哈姆特公会签到
	return $.http.post({ 
		...
		.then((res) => { 
			const body = JSON.parse(res.body); 
			const isAlreadySigned = body.msg && body.msg.includes('已經簽到');
			
			if (body.ok) { 
				return { status: 'success', name: v.name, msg: body.msg }; 
			} else if (isAlreadySigned) {
				return { status: 'already', name: v.name, msg: body.msg }; 
			} else {
				return { status: 'fail', name: v.name, msg: body.msg }; 
			}
		})
		.catch(e => { 
			// 核心优化：拦截并净化 Loon 底层冗长的网络报错
			let errMsg = e.message || String(e);
			if (errMsg.toLowerCase().includes('timeout')) {
				errMsg = '网络请求超时';
			} else if (errMsg.includes('HTTPClient request failed')) {
				errMsg = '网络连接异常';
			}
			return { status: 'fail', name: v.name, msg: errMsg }; 
		});
}
function BahamutAnswer() { //动画疯答题
	...
	return $.http.get({ 
		...
		.then(async (res) => { 
			let r;
			try {
				// 核心修改：增加安全解析，防止被HTML拦截页搞崩溃
				r = JSON.parse(res.body); 
			} catch (err) {
				throw new Error("接口被拦截 (可能触发了网页反爬验证)");
			}
			...
			if (r.token) { 
				...
				$.notifyMsg.push(`⭕ 动画答题丨获得 ${cleanAnswer}`); 
			// 核心修改：拦截已答题的系统回复，不抛出异常
			} else if (r.msg && (r.msg.includes('答過') || r.msg.includes('一天僅限一次機會'))) {
				$.log('', '✅ 今日已答过题了');
				$.notifyMsg.push(`⭕ 动画答题丨今日已答过题了`);
			} else { 
		...
		.catch(e => { 
			...
			if (errorMsg.includes('一天僅限一次機會') || errorMsg.includes('答過')) {
				$.notifyMsg.push(`⭕ 动画答题丨今日已答过题了`); 
			} else {
				$.hasError = true;
				$.notifyMsg.push(`❌ 动画答题, ${errorMsg}`); 
				$.log('', `❌ 动画答题, ${errorMsg}`); 
			}
		});
}
