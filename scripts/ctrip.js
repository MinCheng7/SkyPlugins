...
配置脚本后登陆"携程旅行"微信小程序或"携程网页版"(https://m.ctrip.com/)即可获取账号Cookie。多账号请勿"退出登陆"。
编辑者：@MinCheng7
原作者：@NobyDa
更新时间：2026/04/17
...
!(async () => {
    ...
    $.hasError = false; // 新增：全局错误标记，用于动态生成通知的副标题
    ...
    if (userNum) {
        ...
        for (const i in user.account) {
            const checkinRes = await Checkin(user.account[i].auth);
            const pointsRes = await Points(user.account[i].auth);

            // 状态检测：遇到异常情况打上标记
            if (checkinRes.includes('失败') || checkinRes.includes('失效') || checkinRes.includes('错误')) {
                $.hasError = true;
                if (checkinRes.includes('失效')) invalidUser.push(i);
            }

            // 核心优化：排版视觉调整
            // 如果是多账号，加上 👤 图标和尾号区分；如果是单账号，直接隐藏账号前缀，极致清爽
            const prefix = userNum > 1 ? `👤 账号[${i.slice(-4)}]\n └ ` : '';
            // 使用优雅的「 丨 」符号替代生硬的英文逗号
            const text = `${prefix}${checkinRes}${pointsRes ? ' 丨 ' + pointsRes : ''}`;
        ...
        $.hasError = true;
        notifyMsg.push(`❗ 未获取授权，请先登录获取Cookie`);
    }
})()
    .catch((err) => {
        $.hasError = true; // 捕获到底层报错也标记为异常
        notifyMsg.push(`❌ 运行错误: ${err}`);
        $.error(err);
    })
    .finally(async () => {
        if (notifyMsg.length) {
            // 核心优化：增加动态副标题，让手机屏幕顶部的弹窗一目了然
            const subTitle = $.hasError ? '❗️ 存在异常或失效账号' : '✅ 签到及积分查询完成';
            // 多账号之间增加一个空行，提升阅读体验
           // $.msg(`携程旅行`, subTitle, notifyMsg.join('\n\n'));
...
function Points(key) {
    ...
    return $.http.post(opts)
        .then((resp) => {
            ...
            resp.body = JSON.parse(resp.body?.startsWith('{') && resp.body || '{}');
            // 在 Points 函数中，找到并替换这两个 return 语句：
            if (resp.body.isLogin) {
                return `💰 总积分: ${resp.body.availableCredits || 0}` // 修改这里
            }
        })
        .catch((err) => {
            $.error(`Send points request error:`, err)
            return `💰 总积分: 查询错误` // 修改这里
        })
}
