...
async function Charge(mid, bp_num) {
	...
	// 1. 数据校验与清洗：确保充电B币数是一个合法的正整数，且符合B站最低 2 B币的限制
	const bpToCharge = Math.floor(Number(bp_num)); // 强制转为数字，并向下取整（过滤小数）
	if (isNaN(bpToCharge) || bpToCharge < 2) {
		$.log(`❕ 充电失败: 设置的B币数[${bp_num}]无效，B站规定单次最低充电数为2`);
		return ""; // 停止执行，静默退出
	}
